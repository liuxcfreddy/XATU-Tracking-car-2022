...
/*程序功能说明：正常行驶时候一切正常
但是新增的倒车程序调试与转向助力请注意参数调试
posx参数的功能是当车辆处于转向的时候会加大马达的功率，避免转向时候车辆动力不足
我默认了一个比较适中的值，你不需要去修改，但是如果你能看懂程序也可以自己在ctry()函数内修改

AB参数是版本控制，如果你发现自己的车的转向方向与感应板的感应正好相反，请将
P2^3接口接地，就可以切换成正确的转向版本，如果还是不能够的话，那就试着调换
你的感应板的接入顺序

ik的功能是车辆对轨道丢失的敏感程度，数字越大代表着车辆对丢失轨道的反应越慢
这个值太小了车辆一旦扫不到轨道就会倒车容易“神经质”似乎会过于担心丢失轨道
这个值太大，车就会变得心眼太大，都跑出去好远了才知道倒车，这个值需要你自己
根据自己的车的情况去修改默认是4000，属于比较大的，请你自己修改

ikt是车辆的倒车倍率，值越大，倒车的距离就会越大，最小为2

Zhuan_Jiao[3]数组里储存着由小到大的转向角度，是左右对称的，如果发现舵机转向
不合适要及时调整，避免烧毁

端口设置部分我想应该不难，根据你的需要你可以自己设置

stop是个标志量，作用是，当你把车从地上拿起来的时候他就会自己停车你不用修改
Zhong_Xin_Xiu_Zheng 这是个修正量，如果你的舵机回归90度后你的车轮并没有对正，
可以修改这车参数试试，有正有负，你的偏左偏右请自行实验，因为你的舵机安装方向我也不知道

刹车灯就是P2^1和P2^2之间接个LED就行，倒车就有指示了

至于外部的调节请看说明文档

对了，程序里还有一些神奇的参数等着你发现哦，我留下了一点点小彩蛋，调节得当很有用哦
程序内的寻迹思路希望对你有启发，也可以用在其他的项目与比赛当中，但是请遵守开源协议
*/
#include "intrins.h"
...
unsigned int ik = 4000;            //触发倒车的延迟时间（单位：2微秒_2us）建议不要小于1000
unsigned int ikt = 2;              //倒车时间倍数
unsigned int Servo0PwmDuty = 1500; // 舵机PWM脉冲宽度   1.5ms脉冲宽度  为舵机正中位置 区间为500~2500，对应0~180°
unsigned int Motor0PwmDuty = 2750; //电机PWM初始宽度     


int Zhuan_Jiao[3] = {20, 30, 45}; //最大转向角设置（左右对称，所以只需要一组，依次从小变大）
int Zhong_Xin_Xiu_Zheng = 7;      //单位 度° 偏右就向左修正，减去一个角度。

static bit stop = 1;                      //自动驻车标志参数请勿修改
...
void DaoChe_ZhuChe()
{
    if (i > ik) //轨道丢失防抖，开始倒车
    {
        ...
        if(i<ik+500)
        {
        Motor0PwmDuty = 6000;  //恭喜你发现了一个神奇的参数，如果你的车倒车的时候总是卡死，试着加大它，但是要小于10000！！！
        }
        else{
        Motor0PwmDuty = 300*(pow);//恭喜你又发现了一个参数pow前面的参数调大后会让你的倒车动力翻倍，但是不要大于600
        }
    ...
    } else //正常前进
    {
        if (stop == 1) {
            ...
            LEDA = 0;
            LEDB = 0;
            Motor0PwmDuty = 300 * (pow + posx);//不是吧！这里也能发现，和上面一样，不要超过600，他负责你前进时候的动力
        } else {
            Car_Motor_A1 = 0;
            Car_Motor_B1 = 0;
        }
